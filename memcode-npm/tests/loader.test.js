...
describe('CodeLoader', () => {
    ...
    describe('constructor', () => {
        it('should create loader with correct options', () => {
            const loader = new loader_1.CodeLoader(fixturesPath, ['node_modules', '.git'], ['.ts', '.js'], false, 500);
            expect(loader).toBeInstanceOf(loader_1.CodeLoader);
        });
    });
    describe('load()', () => {
        it('should load files into memory', () => {
            const loader = new loader_1.CodeLoader(fixturesPath, ['node_modules', '.git'], ['.ts', '.js'], false);
        ...
        it('should exclude specified directories', () => {
            const loader = new loader_1.CodeLoader(fixturesPath, ['src'], ['.ts'], false);
            const stats = loader.load();
            // Should not load any files since src is excluded
            expect(stats.fileCount).toBe(0);
        });
        it('should start file watching when enabled', () => {
            const loader = new loader_1.CodeLoader(fixturesPath, ['node_modules'], ['.ts'], true, 500);
            const stats = loader.load();
        ...
        it('should handle non-existent directory gracefully', () => {
            const loader = new loader_1.CodeLoader('/non/existent/path', [], ['.ts'], false);
            const stats = loader.load();
            expect(stats.fileCount).toBe(0);
        });
        it('should load real C source files from input driver', () => {
            // Test with real Linux kernel input driver code
            const loader = new loader_1.CodeLoader(fixturesPath, ['.git', 'node_modules'], ['.c', '.h'], false);
            const stats = loader.load();
    ...
    describe('read()', () => {
        it('should read TypeScript file by relative path', () => {
            const loader = new loader_1.CodeLoader(fixturesPath, [], ['.ts'], false);
            loader.load();
        ...
        it('should read real C files from memory', () => {
            const loader = new loader_1.CodeLoader(fixturesPath, [], ['.c', '.h'], false);
            loader.load();
    ...
    describe('glob()', () => {
        ...
        it('should support ? wildcard in glob', () => {
            const loader = new loader_1.CodeLoader(fixturesPath, [], ['.c'], false);
            loader.load();
            const results = loader.glob('src/?.c');
    ...
    describe('shouldInclude()', () => {
        it('should correctly identify included and excluded files', () => {
            const loader = new loader_1.CodeLoader(fixturesPath, ['node_modules'], ['.ts', '.js'], false);
        ...
        it('should handle nested excluded directories', () => {
            const loader = new loader_1.CodeLoader(fixturesPath, ['node_modules', 'dist'], ['.ts'], false);
    ...
    describe('file watching', () => {
        it('should stop watching without error', () => {
            const loader = new loader_1.CodeLoader(fixturesPath, [], ['.ts'], true, 500);
            loader.load();
        ...
        it('should handle excluded directories in watcher', () => {
            ...
            const loader = new loader_1.CodeLoader(fixturesPath, ['node_modules', '.git'], ['.ts'], true, 500);
            loader.load();
    ...
    describe('error handling', () => {
        it('should handle unreadable directory', () => {
            ...
            try {
                ...
                const loader = new loader_1.CodeLoader(tempDir, [], ['.ts'], false);
                // Should not crash
                const stats = loader.load();
    ...
    describe('real user scenarios', () => {
        it('should load Linux kernel input driver code', () => {
            ...
            const loader = new loader_1.CodeLoader(fixturesPath, ['.git', 'node_modules', '__pycache__'], ['.c', '.h'], false);
            const stats = loader.load();
        ...
        it('should perform full workflow like a real user', () => {
            ...
            const loader = new loader_1.CodeLoader(fixturesPath, ['.git'], ['.c', '.h'], false);
            // 1. Load the code
            const stats = loader.load();
            expect(stats.fileCount).toBeGreaterThan(0);
